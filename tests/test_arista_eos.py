...
class TestArista(unittest.TestCase):
    ...
    def test_show_run_hostname(self):
        async def task():
            for dev in self.devices:
                async with asynchronet.create(**dev) as arista:
        ...
        async def task():
            for dev in self.devices:
                with self.assertRaises(asynchronet.TimeoutError):
                    async with asynchronet.create(**dev, timeout=0.1) as arista:
    ...
    def test_interactive_commands(self):
        async def task():
            for dev in self.devices:
                async with asynchronet.create(**dev) as arista:
                    out = await arista.send_command(
                        "erase startup", pattern=r"\[confirm\]", strip_command=False
                    )
                    out += await arista.send_command("no", strip_command=False)
                    out += await arista.send_command(
                        "show startup", strip_command=False
                    )
